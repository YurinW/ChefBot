...
if __name__ != "__main__" and not os.getenv("CHEFBOT_RUN_INTEGRATION_TESTS"):
    ...
    pytest.skip(
        "Integration-style routing script (set CHEFBOT_RUN_INTEGRATION_TESTS=1 to enable in pytest).",
...
async def run_test_suite():
    ...
    logger.info("=" * 80)
    logger.info("开始 ChefBot Agent 路由测试")
    logger.info("=" * 80)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="ChefBot Agent 路由测试")
    parser.add_argument(
        "--single",
        type=str,
        help="运行单个测试问题（快速调试）",
