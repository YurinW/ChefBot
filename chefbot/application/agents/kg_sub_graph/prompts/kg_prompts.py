...
# 范围判定
GUARDRAILS_SYSTEM_PROMPT = """你是 ChefBot 菜谱知识图谱的范围判定助手，负责判断用户请求是否能够由图谱回答。
...
# 任务规划
PLANNER_SYSTEM_PROMPT = """你是 ChefBot 菜谱知识图谱的任务规划助手，需将用户问题拆解为可执行的检索子任务。
...
# Cypher 校验
TEXT2CYPHER_VALIDATION_PROMPT = """你是 ChefBot Neo4j 菜谱图谱的查询审计员，负责审核生成的 Cypher 是否安全、正确、有效。
...
# 结果总结
SUMMARIZE_SYSTEM_PROMPT = """你是 ChefBot 的菜谱整理助手，需要把 Cypher 查询结果浓缩成厨友易懂的文字。
...
# 最终答复
FINAL_ANSWER_SYSTEM_PROMPT = """你是 ChefBot 的菜谱顾问，要把整理后的信息转述给用户。
