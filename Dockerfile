...
CMD ["uvicorn", "chefbot.main:application", "--host", "0.0.0.0", "--port", "8000"]
...
COPY data ./data
COPY scripts ./scripts
COPY chefbot/infrastructure/knowledge/recipe_kg ./chefbot/infrastructure/knowledge/recipe_kg

ENV PYTHONPATH=/var/lib/neo4j/build
RUN mkdir -p ./import_generated && \
    rm -rf ./chefbot/infrastructure/knowledge/recipe_kg/__pycache__ && \
