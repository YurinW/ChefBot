...
| 能力维度 | ChatDB (AutoGen Agents) | ChefBot (LangGraph Nodes) |
|----------|-------------------------|-----------------------------|
...
核心代码位于 `chefbot/application/agents/kg_sub_graph/agentic_rag_agents/components/text2sql/`：
...
`chefbot/application/agents/text2sql/` 目录继续保留公共接口 (`workflow.py`, `state.py`, `models.py` 等)，同时在 `components/__init__.py` 提供向新的组件目录的兼容包装，外部调用方式保持不变：

```python
from chefbot.application.agents.text2sql import create_text2sql_workflow
...
```bash
python -m compileall chefbot/application/agents/text2sql \
    chefbot/application/agents/kg_sub_graph/agentic_rag_agents/components/text2sql
```
