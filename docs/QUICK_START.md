...
```bash
python3 -c "
from chefbot.config.settings import settings
...
```bash
# 使用爬虫导入（推荐）
python -m chefbot.crawler.cli wikipedia --query "川菜" --import-kb --limit 10
...
# 预期输出：
# NAME                STATUS
# chefbot-server     Up 5 minutes
# chefbot-neo4j      Up 5 minutes
# chefbot-mysql      Up 5 minutes
# chefbot-redis      Up 5 minutes
# chefbot-milvus     Up 5 minutes
# chefbot-etcd       Up 5 minutes
# chefbot-minio      Up 5 minutes
...
# 2. 检查配置
python3 -c "from chefbot.config.settings import settings; print(settings.RERANK_BASE_URL, settings.RERANK_ENDPOINT)"
